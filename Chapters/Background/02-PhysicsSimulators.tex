...
Although being relatively simple to implement, this approach is not always the most efficient, as it requires to solve a \ac{DAE} at each time step, which can be computationally expensive, making it extremely sensitive to the choice of the time step $h$.
...
where $\lambda$ are the coordinates of $m$ one-dimensional point particles having position $\lambda _{j}$, $J$ is the Jacobian and  $g \in Q \times \mathbb{R} \rightarrow \mathbb{R} ^m$ is the configuration coordinate function, and discretizing the time with a fixed time step $h$, we get to:
