...
\begin{figure}
    \centering
    \caption{Velocity of rigid body expressed in terms of spatial vectors. \CarlottaSays{What is the O near $\omega$ ? }}
...
In the case of a rigid body, the velocity of a point $G$ attached to the body with respect to an intertial reference frame attached to an arbitrary point $O$ in the space can be generally expressed by its angular component $\omega$ \CarlottaSays{in which frame is expressed w ? where is it measured ?} about an axis passing through $O$ and its linear component $v _A$, for which the following relation holds:

\begin{equation}
    v _A = \omega \times {}^O R_G
\end{equation}

where ${}^O R_G$ \CarlottaSays{why you use R for a vector ? usually is a rotation matrix. To which set does it belong ? }is the position vector of $G$ with respect to $O$. This holds for any point $G$ on the rigid body. In order to simplify the notation, introducing a Cartesian coordinate frame $\mathcal{O} _{xyz}$, we can define a basis of 6 spatial vectors $\mathcal{D} _O = \{\mathbf{d} _i\} ^6 _{i=1}$ as:

\begin{equation}
    \mathcal{D} _O = \{\mathbf{d} _x, \mathbf{d} _y, \mathbf{d} _z, \mathbf{d} _{O _x}, \mathbf{d} _{O _y}, \mathbf{d} _{O _z} \} \subset \mathcal{M} ^6
\end{equation}

where $\mathcal{M} ^6$ is the space of 6D vectors, defining a Pl\"ucker \CarlottaSays{CItation needed} coordinate system on $\mathcal{M} ^6$ as shown in \cref{fig:pluecker}.
...
\paragraph{Multibody Velocity} Given a floating-base multibody system composed of $n$ rigid bodies, we can define the velocity of the system as the concatenation of the root link \CarlottaSays{What is a root link ? not yet defined} velocity $\mathbf{v}_A$ and the velocities of each body composing the system $\dot{\mathbf{s}} \in \mathbb{R} ^{n}$:
...
\section{Kinematics \CarlottaSays{of what ? }}
...
Given a joint we can define a \textit{joint axis} $\mathbf{a} \in \mathbb{R}^3, |\mathbf{a}| = 1$ with $\mathbf{a}^\wedge \mathbf{a} = \mathbbm{0}_{3 \times 1}$\CarlottaSays{the operator $^\wedge$ and the symbol $\mathbbm{0}$ are not defined, you should define at least in List of Symbols, same for the identity symbol}, in this case, if we recall the definition of the spatial subspace, we can define the joint motion subspace $\boldsymbol{\Phi} _j \in \mathcal{M} ^6$ as:
...
\CarlottaSays{General comment, the equation not inline should be cited in the text.}

\section{Forward Dynamics}
\label{sec:back_fd}

The forward dynamics problem consists of computing the joint accelerations of a multibody system given the joint positions, velocities, and applied forces. As the problem is unsolvable for a single body of the system, the usual approach is to define the constraints that the links must satisfy, write the equation for each link, calculate the local coefficient, and then propagate \CarlottaSays{mention that they are recursive algorithm ? this will make automatic clearer what are you talking about }them in the kinematic tree until a point in which the problem becomes solvable, usually the base of the tree in backpropagation or the end-effector in forward propagation. Although being inertia-matrix algorithms \CarlottaSays{what are inertia-matrix algorithms ? maybe I have missed the definition} much simpler to implement, propagation algorithms involving recursive backsubstition are more efficient in terms of computational cost \CarlottaSays{if you have a reference cite it }, as they can reach the theoretical minimal complexity of $\mathcal{O}(n)$. In this work, the notation $\mathrm{FD}(\cdot)$ will be used to indicate the forward dynamics computation.
...
\begin{figure}[h]
    ...
    \subfloat[Kinematic subtree division.]{
        ...
        \end{tikzpicture}}}
\end{figure}
\CarlottaSays{not important at all, but as suggestion for next time, put the tikz in a separate file to include, makes the latex code more organize and easier to navigate}
The Articulated-Body Algorithm (\ac{ABA}) \CarlottaSays{why both written and as pedix the meaning of the acronym ? } finds its roots by initially considering a subdivision of the kinematic structure in sub-trees as shown in \cref{fig:kin_tree}, i.e. starting from the leaves and going \CarlottaSays{\st{up} down } to the root. If we consider a joint $i$ interacting with the rest of the kinematic chain, it will be subject to an unknown force $\mathbf{f} _i$ defined as:
...
where $\mathbf{I} _i ^A$ is the articulated body inertia, i.e. the inertia \CarlottaSays{\st{felt} you do not actually feel the inertia :) } at the base of the subtree when all its joints are free to move, and $\mathbf{p} ^A _i$ is the associated bias force \CarlottaSays{before $\mathbf{p}$ was a position vector, now is a bias force, confusing}.

In ABA, first, a forward pass is performed to compute the initial articulated body inertia, the initial bias forces, and the link velocities, then a backward pass is performed to compute the articulated body inertia and the bias force at each joint and finally a forward pass computes the acceleration of each body in the kinematic chain. \CarlottaSays{maybe an explanation of the direction of forward and backward  can help e..g leaves-root and viceversa} \CarlottaSays{also explaining that $\lambda(i)$ identifies the parent could help the reader}
...
This yields that once the articulated body inertia and the bias force are known, it is possible to compute the acceleration of each joint independently from the dynamics of the other joints, constituting the main advantage of the articulated-body algorithm. \CarlottaSays{THe algorithm should be cited somewhere in the text !!!! in the algorithm you use $\lambda_i$ but is not specified, double check that all the symbols of the algorithm are explained in the text e.g. what is phi, what is jcalc, what is jtype etc }
