...
In this case, if we recall the definition of the spatial subspace, we can define the joint motion subspace $\mathrm{S} _j \in \mathcal{M} ^6$ as:
...
\begin{equation}
    \boldsymbol{\tau} _i = \mathbf{S} ^T _i \mathbf{f} _i = \mathbf{S} ^T _i (\mathbf{I} _i ^A \mathbf{a} _i + \mathbf{p} ^A _i) = \mathbf{S} ^T _i (\mathbf{I} _i ^A (\mathbf{a} _{\lambda(i)} + \mathbf{S} _i \ddot{\mathbf{s}} _i + \dot{\mathbf{S}} _i \dot{\mathbf{s}} _i)+ \mathbf{p} ^A _i)
\end{equation}

where $\mathbf{S} _i$ is the motion subspace of the $i$-th link, $\mathbf{a} _i$ is the acceleration of the $i$-th link, $\mathbf{a} _{\lambda(i)}$ is the acceleration of the parent link, $\ddot{\mathbf{s}} _i$ is the acceleration of the $i$-th joint, and $\dot{\mathbf{S}} _i$ is the derivative of the motion subspace of the $i$-th link with respect to the joint coordinates.
...
\begin{equation}
    \begin{aligned}
         ...
         &                                               & \frac{1}{2} \ddot{\mathbf{s}} ^T \mathbf{M} (\mathbf{s}) \ddot{\mathbf{s}} + \dot{\mathbf{s}} ^T \mathbf{C} (\mathbf{s}, \dot{\mathbf{s}}) + \mathbf{G} (\mathbf{s}) ^T \ddot{\mathbf{s}} \\
         & \text{subject to}
         &                                               & \mathbf{A} (\mathbf{s}) \ddot{\mathbf{s}} = \mathbf{b} (\mathbf{s}, \dot{\mathbf{s}})
    \end{aligned}
\end{equation}

where $\mathbf{A} (\mathbf{s})$ is the constraint matrix and $\mathbf{b} (\mathbf{s}, \dot{\mathbf{s}})$ is the constraint vector.

The constraint matrix $\mathbf{A} (\mathbf{s})$ is defined as:

\begin{equation}
    \mathbf{A} (\mathbf{s}) = \begin{bmatrix}
        \mathbf{S} _1 ^T \mathbf{M} (\mathbf{s}) \mathbf{S} _1   & \mathbf{S} _1 ^T \mathbf{M} (\mathbf{s}) \mathbf{S} _2   & \dots  & \mathbf{S} _1 ^T \mathbf{M} (\mathbf{s}) \mathbf{S} _{n}   \\
        \mathbf{S} _2 ^T \mathbf{M} (\mathbf{s}) \mathbf{S} _1   & \mathbf{S} _2 ^T \mathbf{M} (\mathbf{s}) \mathbf{S} _2   & \dots  & \mathbf{S} _2 ^T \mathbf{M} (\mathbf{s}) \mathbf{S} _{n}   \\
        \vdots                                                   & \vdots                                                   & \ddots & \vdots                                                     \\
        \mathbf{S} _{n} ^T \mathbf{M} (\mathbf{s}) \mathbf{S} _1 & \mathbf{S} _{n} ^T \mathbf{M} (\mathbf{s}) \mathbf{S} _2 & \dots  & \mathbf{S} _{n} ^T \mathbf{M} (\mathbf{s}) \mathbf{S} _{n} \\
    \end{bmatrix}
\end{equation}

where $\mathbf{S} _i$ is the motion subspace of the $i$-th link. And the constraint vector $\mathbf{b} (\mathbf{s}, \dot{\mathbf{s}})$ is defined as:

\begin{equation}
    \mathbf{b} (\mathbf{s}, \dot{\mathbf{s}}) = \begin{bmatrix}
        \mathbf{S} _1 ^T \mathbf{C} (\mathbf{s}, \dot{\mathbf{s}}) + \mathbf{G} (\mathbf{s})   \\
        \mathbf{S} _2 ^T \mathbf{C} (\mathbf{s}, \dot{\mathbf{s}}) + \mathbf{G} (\mathbf{s})   \\
        \vdots                                                                                 \\
        \mathbf{S} _{n} ^T \mathbf{C} (\mathbf{s}, \dot{\mathbf{s}}) + \mathbf{G} (\mathbf{s}) \\
    \end{bmatrix}
\end{equation}

The constraint matrix $\mathbf{A} (\mathbf{s})$ is a symmetric matrix, and it is positive definite if the robot is fully actuated. The constraint vector $\mathbf{b} (\mathbf{s}, \dot{\mathbf{s}})$ is a vector of dimension $n$.
...
\begin{equation}
    \mathbf{a} ^{constrained} _i = \mathbf{S} _i \ddot{\mathbf{s}} _i + \underbrace{\mathbf{v} _i \times ^* \mathbf{S} _i \dot{\mathbf{s}}} _{\mathbf{c} _i}
\end{equation}

where $\mathbf{c} _i$ is the Coriolis acceleration. The equation for the rotor is then:

\begin{equation}
    {} ^M \mathbf{a} ^{constrained} _i = {} ^M \mathbf{S} _i \ddot{\mathbf{s}} _i + \underbrace{{} ^M \mathbf{v} _i \times ^* {} ^M\mathbf{S} _i \dot{\mathbf{s}}} _{{} ^M \mathbf{c} _i}
\end{equation}
...
\begin{align}
    ...
    \text{where }                           & \qquad \mathbf{a} _i = \mathbf{a} _{\lambda (i)} + \mathbf{S} _i \ddot{\mathbf{s}} _i + \mathbf{c} _i \text{ and } {} ^M \mathbf{a} _i = \mathbf{a} _{\lambda (i)} + {} ^M  \mathbf{S} _i \ddot{\mathbf{s}} _i + {} ^M \mathbf{c} _i \nonumber \\
\end{align}
...
\begin{align}
    \frac{\partial J _1}{\partial \ddot{\mathbf{s}} _i} & = 2 \mathbf{S} ^T _i \mathbf{I} ^A _i \mathbf{S} _i \ddot{\mathbf{s}} _i + 2 \mathbf{S} ^T _i \mathbf{I} ^A _i (\mathbf{a} _{\lambda (i)} + \mathbf{c} _i) - 2 \mathbf{S} ^T _i \mathbf{I} ^A _i \mathbf{a} ^{uc} _i \nonumber                                        \\
    \frac{\partial J _2}{\partial \ddot{\mathbf{s}} _i} & = 2 {} ^M \mathbf{S} ^T _i \mathbf{I} ^M _i {} ^M \mathbf{S} _i \ddot{\mathbf{s}} _i + 2 {} ^M  \mathbf{S} ^T _i \mathbf{I} ^M _i (\mathbf{a} _{\lambda (i)} + {} ^M  \mathbf{c} _i) - 2 {} ^M \mathbf{S} ^T _i \mathbf{I} ^M _i {} ^M \mathbf{a} ^{uc} _i  \nonumber
\end{align}
...
\begin{equation}
    \ddot{\mathbf{s}} _i = \mathbf{D} _i ^{-1} (\mathbf{S} ^T _i \mathbf{I} ^A _i (\mathbf{a} _{\lambda (i)} + \mathbf{c} _i - \mathbf{a} ^{uc} _i) + {} ^M \mathbf{S} ^T _i \mathbf{I} ^M _i ( \mathbf{a} _{\lambda (i)} + {} ^M \mathbf{c} _i - {} ^M \mathbf{a} ^{uc} _i))
\end{equation}
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
        \FOR{$i = 1 \text{ to } n$}
        \STATE $[\mathbf{X}_J, \mathbf{S}_i] = \text{jcalc}(\text{jtype}(i), \dot{\mathbf{s}}_i)$
        \STATE $\mathrm{\mathbf{v}}_J = \mathbf{S}_i \dot{\mathbf{s}}_i$
