...
With these hypotheses, we can reformulate the motion subspace definition reported in \cref{eqn:subspace} for a the rotor attached to joint $i$ as:
...
Beginning with the Gaussian principle of least constraint, the objective is to determine the joint accelerations $\ddot{\mathbf{s}}$ that fulfill the equation of motion as described in \ref{eqn:equation_of_motion}. This task can be framed as a constrained optimization problem:
...
\begin{figure}
    ...
    \subfloat[ABA]{
        \resizebox{0.5\textwidth}{!}{
            \input{Images/TikZ/aba_comparison_aba.tex}
        }}
    \subfloat[Inertia-Matrix-Based]{
        \resizebox{0.5\textwidth}{!}{
            \input{Images/TikZ/aba_comparison_imb.tex}
        }}
\end{figure}
